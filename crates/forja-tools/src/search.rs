...
impl Tool for SearchTool {
    ...
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            ...
            description: "Search the web for real-time information. Useful for up-to-date facts."
                .to_string(),
    ...
    ) -> Result<serde_json::Value, forja_core::error::ForjaError> {
        let query = arguments
            .get("query")
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                forja_core::error::ForjaError::ToolError("Missing 'query' parameter.".to_string())
            })?;
