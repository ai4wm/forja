...
const SHUTDOWN_DOUBLE_TAP_WINDOW: Duration = Duration::from_millis(1_500);
...
mod tests {
    ...
    async fn shutdown_signal_requires_new_double_tap_after_window_expires() {
        ...
        assert!(!signal.trigger());
        tokio::time::sleep(SHUTDOWN_DOUBLE_TAP_WINDOW + std::time::Duration::from_millis(100)).await;
